<x-app-layout>
    ...
    <div x-data="{ editModal: false, selectedCustomer: {}, deleteModal: false, selectedCustomerId: null }" >
        <div class="py-12">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button @click="editModal = true; selectedCustomer = {{ $customer }}" class="text-indigo-600 hover:text-indigo-900">
                                        <i data-lucide="edit" class="w-5 h-5"></i>
                                    </button>
                                    <button @click="deleteModal = true; selectedCustomerId = {{ $customer->id }}" class="text-red-600 hover:text-red-900">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <div>
                                            ...
                                            <input type="text" name="name" id="editName" x-model="selectedCustomer.name" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        </div>
                                        <div>
                                            <label for="editEmail" class="block text-sm font-medium text-gray-700">Email</label>
                                            <input type="email" name="email" id="editEmail" x-model="selectedCustomer.email" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        </div>
                                        <div>
                                            <label for="editPhone" class="block text-sm font-medium text-gray-700">Telepon</label>
                                            <input type="text" name="phone" id="editPhone" x-model="selectedCustomer.phone_number" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        </div>
                                        <div>
                                            <label for="editAddress" class="block text-sm font-medium text-gray-700">Alamat</label>
                                            <input type="text" name="address" id="editAddress" x-model="selectedCustomer.address" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div x-show="deleteModal" class="fixed z-10 inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form :action="`/customers/${selectedCustomerId}`" method="POST">
                        @csrf
                        @method('DELETE')
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
